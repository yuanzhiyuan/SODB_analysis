...
# 运行代码：python run_CCST.py --data_type posterior --data_name V1_Mouse_Brain_Sagittal_Posterior --lambda_I 0.3 --DGI 1 --load 0
...
def CCST_on_posterior(args):
    ...
    if args.cluster:
        ...
        adata_spatial = sc.read_h5ad('/home/workspace2/zhaofangyuan/CCST/CCST/dataset/V1_Mouse_Brain_Sagittal_Posterior/V1_Mouse_Brain_Sagittal_Posterior.h5ad')
        ...
        eval_resolution_list = [0.1,0.5,1,1.5,2,2.5,3,3.5,4,4.5]
        if cluster_type == 'leiden':
            for i in range(10):
                adata_copy=adata.copy()
                sc.tl.leiden(adata_copy, key_added="CCST_leiden", resolution=eval_resolution_list[i])
                sc.pl.spatial(adata_copy, color='CCST_leiden', show=False)

                plt.savefig(f'{args.result_path}/CCST_leiden_plot_{i+1}.jpg', bbox_inches='tight', dpi=150)
                adata.obs['CCST_leiden_{}'.format(i+1)]=adata_copy.obs['CCST_leiden']
            adata.write(f'{args.result_path}/CCST_leiden_V1_Mouse_Brain_Sagittal_Posterior(resolution).h5ad')
            print(adata)
        if cluster_type == 'louvain':
            for i in range(10):
                adata_copy=adata.copy()
                sc.tl.louvain(adata_copy, key_added="CCST_louvain", resolution=eval_resolution_list[i])
                sc.pl.spatial(adata_copy, color='CCST_louvain', show=False)

                plt.savefig(f'{args.result_path}/CCST_louvain_plot_{i+1}.jpg', bbox_inches='tight', dpi=150)
                adata.obs['CCST_louvain_{}'.format(i+1)]=adata_copy.obs['CCST_louvain']
            adata.write(f'{args.result_path}/CCST_louvain_V1_Mouse_Brain_Sagittal_Posterior(resolution).h5ad')
            print(adata)
